...
nimble_port_freertos_init(TaskFunction_t host_task_fn)
{
     ...
     */
    xTaskCreate(host_task_fn, "ble", configMINIMAL_STACK_SIZE*2 + 400,
                NULL, tskIDLE_PRIORITY + 1, &host_task_h);
}

#include "app_log.h"

void nimble_port_log_output(char *lvl, const char *format, ...)
{
    APP_LOG_RAW_INFO("<%s>  ", lvl);
    va_list args;
    va_start(args, format);

    APP_LOG_RAW_INFO(format, args);
